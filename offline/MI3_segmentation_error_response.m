function  MI3_segmentation_error_response(recordingFolder)
...
% This version is for find error response, use trigers 3 and 4
% [recordingFolder] - where the EEG (data & meta-data) are stored.
% befor running MI3 for error response, you should run create_trainingVec 
...
for i = 1:length(events)
    if strcmp('3.000000000000000',events(i).type)               % find trial start marker
        marker1Index(i) = 1     ;                              % index markers
    elseif strcmp('4.000000000000000',events(i).type)               % find trial start marker
        marker1Index(i) = 1     ;                              % index markers
    else
        marker1Index(i) = 0;
    end
